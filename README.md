...
One MuJoCo bench for the SO-ARM100 arm (four tasks, scripted IK expert, domain randomisation, six held-out physics cells, LeRobot recorder) and three ways to learn on it: curriculum PPO, imitation (BC, DAgger, ACT), and a language-conditioned policy tested on held-out paraphrases and unseen task–colour pairs against SmolVLA-base zero-shot. 122 tests green in CI. The expert scores 0.93 on lift and pick-and-place, 0.73 and 0.72 on the small cube; policy tables not yet measured.
